  ...
  $.fn.tokenField = function(options) {
    ...
    function observeTokenField(tokenField) {
      ...
      $('.token-input input', tokenField)
        ...
        .focus(function() {
          $(this).next('.token-input-sizer').html($(this).val()+'###');
          tokenField.addClass('token-field-focused');
          return true;
        })
          ...
          if (!$(this).data('blur')) {
            ...
            if (settings.max == 0 || $(this).closest('.token-input').siblings('.token').length < settings.max) {
              if (isToken($(this).val())) {
                ...
                $(this).val('');
                tokenField.addClass('token-field-not-empty');
              } else {
            ...
            tokenField.removeClass('token-field-focused');
            if (!tokenField.find('input:hidden').val()) {
              tokenField.removeClass('token-field-not-empty');
            }
          }
          return true;
        });
    ...
    function removeToken(token) {
      ...
      if (!input.val()) {
        token.closest('.token-field').removeClass('token-field-not-empty');
      }
      token.remove();
    }
